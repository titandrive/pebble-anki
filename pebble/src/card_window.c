...
//  y=16  ├──────────────────────────────┤
//        │                              │
//        │  card text (scrollable)      │  clipped viewport
//        │                              │
//  y=-40 ├──────────────────────────────┤
...
static Layer     *s_clip_layer;     // scroll viewport — children clip to its bounds
static TextLayer *s_content_layer;  // inside clip, shifted for scrolling
...
static void prv_apply_scroll(void) {
  GRect clip = layer_get_bounds(s_clip_layer);
  int max_scroll = MAX(0, s_content_h + PAD - s_scroll_area_h);
  ...
  layer_set_frame(text_layer_get_layer(s_content_layer),
    GRect(PAD, PAD - s_scroll_offset, clip.size.w - PAD * 2, s_content_h));
}

static void prv_load_text(const char *text) {
  s_scroll_offset = 0;
  GRect clip = layer_get_bounds(s_clip_layer);
  int content_w = clip.size.w - PAD * 2;
  GFont font = fonts_get_system_font(FONT_KEY_GOTHIC_18);
  GSize sz = graphics_text_layout_get_content_size(
    text, font, GRect(0, 0, content_w, 2000),
    GTextOverflowModeWordWrap, GTextAlignmentLeft
  );
  s_content_h = sz.h + PAD;
...
static void prv_window_load(Window *win) {
  ...
  // Clip layer (scroll viewport)
  s_clip_layer = layer_create(GRect(0, HINT_H, w, s_scroll_area_h));
  layer_add_child(root, s_clip_layer);

  // Content TextLayer — inside clip layer, y shifted for scrolling
  s_content_layer = text_layer_create(GRect(PAD, PAD, w - PAD * 2, 1000));
...
static void prv_window_unload(Window *win) {
  ...
  layer_destroy(s_clip_layer);      // also destroys s_content_layer as child
  text_layer_destroy(s_prompt_layer);
  s_hint_layer = s_clip_layer = s_prompt_layer = NULL;
